...
class MessagesSpec extends Specification {
  ...
  val api = {
    ...
    new DefaultMessagesApiProvider(env, config, langs) {
      override def loadAllMessages = testMessages
    }.get
  }
    ...
    "report error for invalid lang" in {
      {
        ...
        val messagesApi = new DefaultMessagesApiProvider(new Environment(new File("."), this.getClass.getClassLoader, Mode.Dev), Configuration.reference, langs).get
