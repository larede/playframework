...
import play.api.i18n.{ DefaultLangs, DefaultLangsProvider, DefaultMessagesApi, DefaultMessagesApiProvider }
...
class FormSpec extends Specification {
  ...
  "correctly lookup error messages when using errorsAsJson" in {
    val messagesApi = new DefaultMessagesApiProvider(Environment.simple(), Configuration.reference, new DefaultLangsProvider(Configuration.reference).get).get
